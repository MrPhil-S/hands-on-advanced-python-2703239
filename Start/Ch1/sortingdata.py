...
#pprint.pprint(sorted_dataset)

# TODO: method 2: use the sort() function that every list has to sort in-place
dataset.sort(key=lambda d: d['snow'], reverse=False)
#pprint.pp(dataset)

# TODO: Sort on multiple fields: first by snowfall, then by average wind speed descending
dataset.sort(key=lambda d: (d['snow'], -d['awnd']) ) 
pprint.pp(dataset)
